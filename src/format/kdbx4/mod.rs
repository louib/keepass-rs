...
mod kdbx4_tests {
    ...
    fn test_with_config(config: DatabaseConfig) {
        ...
        let database_key = DatabaseKey::new().with_password(&password);

        let mut encrypted_db = Vec::new();
        dump_kdbx4(&db, database_key.clone(), &mut encrypted_db).unwrap();

        let decrypted_db = parse_kdbx4(&encrypted_db, database_key).unwrap();
    ...
    pub fn header_attachments() {
        ...
        let database_key = DatabaseKey::new().with_password("test");
