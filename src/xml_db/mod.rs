...
mod tests {
    ...
    pub fn test_entry() {
        ...
        let database_key = make_key();

        let mut encrypted_db = Vec::new();
        kdbx4::dump_kdbx4(&db, database_key.clone(), &mut encrypted_db).unwrap();
        let decrypted_db = kdbx4::parse_kdbx4(&encrypted_db, database_key).unwrap();
