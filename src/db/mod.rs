...
impl Database {
    ...
    pub fn open(
        ...
        match database_version {
            DatabaseVersion::KDB(_) => parse_kdb(data.as_ref(), &key.get_key_elements()?),
            DatabaseVersion::KDB2(_) => Err(DatabaseOpenError::UnsupportedVersion.into()),
            DatabaseVersion::KDB3(_) => parse_kdbx3(data.as_ref(), &key.get_key_elements()?),
    ...
    pub fn get_xml(
        ...
        let data = match database_version {
            ...
            DatabaseVersion::KDB3(_) => decrypt_kdbx3(data.as_ref(), &key.get_key_elements()?)?.2,
