...
impl Database {
    ...
    /// Open a database from a std::io::Read, allowing for async operations.
    pub fn open_async(
        source: &mut dyn std::io::Read,
        key: DatabaseKey,
    ) -> Result<Database, DatabaseOpenError> {
        ...
        match database_version {
            DatabaseVersion::KDB(_) => Err(DatabaseOpenError::UnsupportedVersion.into()),
            DatabaseVersion::KDB2(_) => Err(DatabaseOpenError::UnsupportedVersion.into()),
            DatabaseVersion::KDB3(_) => Err(DatabaseOpenError::UnsupportedVersion.into()),
            DatabaseVersion::KDB4(_) => parse_kdbx4_async(data.as_ref(), &key),
        }
    }
